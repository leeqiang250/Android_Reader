...
public class VideoView extends SurfaceView
{
	...
	private SZFile currentPlayFile; // 当前播放文件
	...
	private final SurfaceHolder.Callback callback = new SurfaceHolder.Callback()
	{
		...
		public void surfaceDestroyed(SurfaceHolder holder)
		{
			...
			if (currentPlayFile != null)
				SPUtil.save(currentPlayFile.getContentId(), getCurrentProgress());
			release_();
		}
	};
	...
	public boolean hasPermitt()
	{
		if (!isStart)
			throw new IllegalStateException(
					"required call method 'VideoView.start(drmFile)' first.");
		return hasPermit;
	}
	...
	private void initPlayer()
	{
		...
		mediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "overlay-format",
	 ...
	 * @param drmFile
	 */
	public void start(SZFile drmFile)
	{
		...
		if (currentPlayFile != null)
		{
			if (!currentPlayFile.isCheckOpen()
					|| TextUtils.isEmpty(currentPlayFile.getCek_cipher_value()))
			{
				// 没有权限
				hasPermit = false;
			...
			{
				initPlayer();
				mediaPlayer.setDataSource(currentPlayFile.getFilePath(),
						currentPlayFile.getCek_cipher_value());
		...
		public boolean onInfo(IMediaPlayer mp, int what, int extra)
		{
			switch (what)
			{
				case IMediaPlayer.MEDIA_INFO_BUFFERING_START:
					sendMsg(MediaHandler.STATE_BUFFERING, MediaHandler.POSITIVE, 0, null);
					break;

				case IMediaPlayer.MEDIA_INFO_BUFFERING_END:
					sendMsg(MediaHandler.STATE_BUFFERING, MediaHandler.NEGATIVE, 0, null);
					break;
	...
	private final OnPreparedListener preparedListener = new OnPreparedListener()
	{
		...
		public void onPrepared(IMediaPlayer mp)
		{
			...
			sendMsg(MediaHandler.STATE_PREPARED_PLAY, 0, duration, currentPlayFile);
			if (mediaPlayer != null)
			{
				// 当前播放进度
				currentProgress = (long) SPUtil.get(currentPlayFile.getContentId(), 0L);
	...
	private final tv.danmaku.ijk.media.player.IMediaPlayer.OnVideoSizeChangedListener mSizeChangedListener = new tv.danmaku.ijk.media.player.IMediaPlayer.OnVideoSizeChangedListener()
	{
		@Override
				...
				int sarDen)
		{
			...
			SZLog.w(TAG, "mpVideo: width = " + mVideoWidth + ", height = " + mVideoHeight);
			SZLog.w(TAG, "mpVideo: sarNum = " + sarNum + ", sarDen = " + sarDen);

			if (mVideoHeight > 0 && mVideoWidth > 0)
			{
				Point mPoint = DeviceUtil.getScreenSize(getContext());
				...
				SZLog.w(TAG, "VideoWidth = " + mVideoWidth + ", VideoHeight = " + mVideoHeight);

				// int margin = (mPoint.x - mVideoWidth) / 2;
				// DebugLog.e(TAG, "margin= " + margin);
				FrameLayout.LayoutParams lp = new FrameLayout.LayoutParams(mVideoWidth,
						mVideoHeight);
				// lp.setMargins(margin, 0, margin, 0);
				lp.gravity = Gravity.CENTER;
				setLayoutParams(lp);
				//@REMOVE: mSurfaceHolder.setFixedSize(mVideoWidth, mVideoHeight);
			}
		}
	};
	...
	private final OnCompletionListener onCompletionListener = new OnCompletionListener()
	{
		...
		{
			SZLog.e(TAG, "video play complete.");
	 ...
	 * @param what
	 * @param arg1
	 *            long参数1,putLong("arg1", arg1);
	 * @param arg2
	 *            long参数2,putLong("arg2", arg2);
	 * @param obj
	 */

	private final void sendMsg(int what, long arg1, long arg2, Object obj)
	{

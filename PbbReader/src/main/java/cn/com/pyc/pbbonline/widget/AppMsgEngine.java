 ...
 */
class AppMsgEngine extends Handler implements Comparator<AppMsg> {
    ...
    private static WeakHashMap<Activity, AppMsgEngine> sManagers;
    ...
    private AppMsgEngine() {
        msgQueue = new PriorityQueue<AppMsg>(1, this);
        stickyQueue = new LinkedList<AppMsg>();
    }

    /**
     * @return A {@link AppMsgEngine} instance to be used for given {@link android.app.Activity}.
     */
    static synchronized AppMsgEngine obtain(Activity activity) {
        if (sManagers == null) {
            sManagers = new WeakHashMap<Activity, AppMsgEngine>(1);
        }
        AppMsgEngine manager = sManagers.get(activity);
        if (manager == null) {
            manager = new AppMsgEngine();
    ...
    static synchronized void release(Activity activity) {
        if (sManagers != null) {
            final AppMsgEngine manager = sManagers.remove(activity);
            if (manager != null) {
                manager.clearAllMsg();
            }
        }
    ...
    static synchronized void clearAll() {
        if (sManagers != null) {
            final Iterator<AppMsgEngine> iterator = sManagers.values().iterator();
            while (iterator.hasNext()) {
                final AppMsgEngine manager = iterator.next();
                if (manager != null) {
                    manager.clearAllMsg();
                }
                iterator.remove();

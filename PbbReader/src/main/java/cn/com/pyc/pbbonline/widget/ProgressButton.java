...
public class ProgressButton extends HighlightButton
{
	...
	//有点运动状态,处理中
	public static final int IN_HANDING = 2;
	...
	private void initAttrs(Context context, AttributeSet attrs)
	{
		...
		mBackgroundColor = a.getColor(R.styleable.ProgressButton_progressbtn_backgroud_color,
				Color.parseColor("#6699ff"));
		mBackgroundSecondColor = a.getColor(
				R.styleable.ProgressButton_progressbtn_backgroud_second_color, Color.LTGRAY);
		//mButtonRadius = a.getFloat(R.styleable.ProgressButton_progressbtn_radius, getMeasuredHeight() / 2);
		float defValue = getResources().getDimension(R.dimen.progressbtn_conrer_radius);
		mButtonRadius = a.getDimension(R.styleable.ProgressButton_progressbtn_radius, defValue);
		mTextColor = a
				.getColor(R.styleable.ProgressButton_progressbtn_text_color, mBackgroundColor);
		mTextSize = a.getDimension(R.styleable.ProgressButton_progressbtn_text_size, 30f);
		mTextCoverColor = a.getColor(R.styleable.ProgressButton_progressbtn_text_covercolor,
				Color.WHITE);
		a.recycle();
	}
	...
	private void init()
	{
		...
		GradientDrawable mBackDrawable = (GradientDrawable) getResources().getDrawable(
				R.drawable.progressbtn_rect_bg).mutate();
	...
	private void drawBackground(Canvas canvas)
	{
		...
		mBackgroundBounds.left = 1;
		mBackgroundBounds.top = 1;
		mBackgroundBounds.right = getMeasuredWidth() - 1;
		mBackgroundBounds.bottom = getMeasuredHeight() - 1;
		...
		switch (mState)
		{
			...
			case DOWNLOADING:
				...
				mProgressBgGradient = new LinearGradient(0, 0, getMeasuredWidth(), 0, new int[]
				{ mBackgroundColor, mBackgroundSecondColor }, new float[]
				{ mProgressPercent, mProgressPercent + 0.001f }, Shader.TileMode.CLAMP);
				mBackgroundPaint.setColor(mBackgroundColor);
				mBackgroundPaint.setShader(mProgressBgGradient);
				canvas.drawRoundRect(mBackgroundBounds, mButtonRadius, mButtonRadius,
						mBackgroundPaint);
				break;
			case IN_HANDING:
				mBackgroundPaint.setShader(null);
	...
	private void drawTextAbove(Canvas canvas)
	{
		final float y = canvas.getHeight() / 2
				- (mTextPaint.descent() / 2 + mTextPaint.ascent() / 2);
		...
		switch (mState)
		{
			case NORMAL:
				...
				canvas.drawText(mCurrentText.toString(), (getMeasuredWidth() - textWidth) / 2, y,
						mTextPaint);
				break;
			case DOWNLOADING:
				...
				{
					mProgressTextGradient = new LinearGradient(
							(getMeasuredWidth() - textWidth) / 2, 0,
							(getMeasuredWidth() + textWidth) / 2, 0, new int[]
							{ mTextCoverColor, mTextColor }, new float[]
							{ textProgress, textProgress + 0.001f }, Shader.TileMode.CLAMP);
					mTextPaint.setColor(mTextColor);
			...
			case IN_HANDING:
				...
				canvas.drawText(mCurrentText.toString(), (getMeasuredWidth() - textWidth) / 2, y,
						mTextPaint);
				canvas.drawCircle((getMeasuredWidth() + textWidth) / 2 + 4 + mDot1transX, y, 4,
						mDot1Paint);
				canvas.drawCircle((getMeasuredWidth() + textWidth) / 2 + 24 + mDot2transX, y, 4,
						mDot2Paint);
				break;
	...
	public void setState(int state)
	{
		...
		{//状态确实有改变
			this.mState = state;
			invalidate();
			if (state == ProgressButton.IN_HANDING)
			{
				//开启两个点动画
	...
	public void setProgressText(String text, float progress)
	{
		...
		{
			mCurrentText = text + getResources().getString(R.string.downloaded_tip, (int) progress);
		...
		else if (progress > mMaxProgress)
		{
			mProgress = 100;
			mCurrentText = text
					+ getResources().getString(R.string.downloaded_tip, (int) mProgress);
			invalidate();
		}
	}

...
/**
 * 分段控件，https://github.com/7heaven/SHSegmentControl <br/>
 * <br/>
 * set segmentControl's property using attrs,using '|' to separate segments.
 * 
 * @author Created by 7heaven on 15/4/22.
 */
public class SegmentControl extends View
{
	...
	public SegmentControl(Context context, AttributeSet attrs, int defStyle)
	{
		...
		mCornerRadius = ta.getDimensionPixelSize(R.styleable.SegmentControl_cornerRadiuss,
				(int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, context
						.getResources().getDisplayMetrics()));
		...
		int gap = ta.getDimensionPixelSize(R.styleable.SegmentControl_gaps, (int) TypedValue
				.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, context.getResources()
						.getDisplayMetrics()));
		...
		mSelectedTextColor = ta
				.getColor(R.styleable.SegmentControl_textSelectedColors, Color.WHITE);
		if (mColors == null)
		{
		...
		mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SegmentControl_stroke_outner_width,
				mStrokeWidth);
		mStrokeInnerWidth = ta.getDimensionPixelSize(R.styleable.SegmentControl_stroke_inner_width,
				mStrokeInnerWidth);
	...
	{
		mPaint.setTextSize((int) (TypedValue.applyDimension(unit, textSize, getContext()
				.getResources().getDisplayMetrics())));
	...
	public void setSelectedIndex(int index)
	{
		...
		if (index < 0 || index > mTexts.length - 1)
		...
		if (mTexts != null && mTexts.length > 0)
		{
			for (int i = 0; i < mTexts.length; i++)
			{
				...
				if (i == mCurrentIndex && mSelectedDrawable != null)
				{
					...
					mSelectedDrawable.setRadius(topLeftRadius, topRightRadius, bottomLeftRadius,
							bottomRightRadius);
				...
				canvas.drawText(mTexts[i], mCacheBounds[i].left
						+ (mSingleChildWidth - mTextBounds[i].width()) / 2, mCacheBounds[i].top
						+ ((mSingleChildHeight + mTextBounds[i].height()) / 2), mPaint);
			}
		}
	}

...
//		isAudition = getId() == R.id.amr_view_wave_audition;
	}
	...
	// recordProgress = 0;
	// curRecord.x = 0;
	// _SysoXXX.message("path empty：" + wavePath.isEmpty());
	// }
	// }
	...
	{
		pause = false;
		// _SysoXXX.message("progress=" + progress);
		if (progress == 0)
		{
			curRecord.x = 0;
			wavePath.reset();
	...
	protected void onDraw(Canvas canvas)
	{
		...
		// _SysoXXX.message("null:" + (fff == null));
		// if (fff == null)
		// {
		// return;
		// }
		//
		// if (mPoints == null || mPoints.length < fff.length * 4)
		// {
		// mPoints = new float[fff.length * 4];
		...
		// mPoints[i * 4 + 3] = 2 + mRect.height() / 2 + fff[i];
		// }
		...
		if (isAudition)
		{
			...
			{
				canvas.drawText(formatTime(), curRecord.x + POINT_SPACE,
						halfHeight - 25, mPaint);
//				canvas.drawLine(curRecord.x + X_INCREMENT, 0, curRecord.x
//						+ X_INCREMENT, height, mPaint);
			}
		}

		curRecord.x += POINT_SPACE;
		if (curRecord.x > width)
		{
			curRecord.x = 0;
			wavePath.reset();
		}
		getRhombusPath(curRecord, X_INCREMENT, ratio * halfHeight);
		canvas.drawPath(wavePath, mPaint);

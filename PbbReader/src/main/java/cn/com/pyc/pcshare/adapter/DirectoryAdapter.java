package cn.com.pyc.pcshare.adapter;
...
public class DirectoryAdapter extends BaseAdapter {
    ...
    public DirectoryAdapter(Context context, List<SmbFile> files) {
        ...
        folderImg = mContext.getResources().getDrawable(R.drawable.format_folder);
        fileImg = mContext.getResources().getDrawable(R.drawable.format_file);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = View.inflate(mContext, R.layout.pc_item_list_direcory, null);
        ...
        try {
            ...
            if (file.isDirectory()) {
                ...
                holder.info.setText(DataConvert.toSize(mContext, file.length()));
            } else {
                holder.icon.setImageDrawable(null);
            }
            String fileName = file.getName();
            if (fileName != null) {
                if (fileName.endsWith(".pbb")) {
                    holder.name.setText(fileName.substring(0, fileName.length() - 4));
                } else {
                    holder.name.setText(fileName);
                }
            }
            if (file.getLastModified() > 0)
                holder.time.setText(DataConvert.toDate(file.lastModified()));

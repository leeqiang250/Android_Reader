...
public final class ViewfinderView extends View
{
	...
	public ViewfinderView(Context context, AttributeSet attrs)
	{
		...
		if (mBitmapLine == null)
			mBitmapLine = ((BitmapDrawable) getResources().getDrawable(
					R.drawable.qrcode_scanner_line)).getBitmap();
	...
	public void onDraw(Canvas canvas)
	{
		...
		{
			return; // not ready yet, early draw before
					// done configuring
		}
	...
	{
		int color = Color.argb(255, 55, 183, 94);
		paint.setColor(color);
		// paint.setColor(Color.WHITE);
		paint.setAlpha(OPAQUE);

		canvas.drawRect(frame.left - 5, frame.top - 5, frame.left + frame.width() / 9,
				frame.top + 5, paint);
		canvas.drawRect(frame.left + (frame.width() * 8) / 9, frame.top - 5, frame.right,
				frame.top + 5, paint); // top

		canvas.drawRect(frame.left - 5, frame.top, frame.left + 5, frame.top + frame.height() / 9,
				paint);
		canvas.drawRect(frame.left - 5, frame.top + (frame.height() * 8) / 9, frame.left + 5,
				frame.bottom, paint); // left

		canvas.drawRect(frame.right - 5, frame.top - 5, frame.right + 5, frame.top + frame.height()
				/ 9, paint);
		canvas.drawRect(frame.right - 5, frame.top + (frame.height() * 8) / 9, frame.right + 5,
				frame.bottom, paint); // right

		canvas.drawRect(frame.left - 5, frame.bottom - 5, frame.left + frame.width() / 9,
				frame.bottom + 5, paint);
		canvas.drawRect(frame.left + (frame.width() * 8) / 9, frame.bottom - 5, frame.right + 5,
				frame.bottom + 5, paint); // bottom
